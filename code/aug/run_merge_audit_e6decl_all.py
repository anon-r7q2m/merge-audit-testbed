...
OUT = os.path.join(EXP, "results/e6pilot", "e6decl_all_report.json")
...
def main():
    ...
    dev = "cuda"
    import glob
    models = [("v1", os.path.join(EXP, "results/e6pilot", "final.pt")),
              ("v3", os.path.join(EXP, "results/e6v3", "final.pt")),
              ("base", None)]
    for d in sorted(glob.glob(os.path.join(EXP, "results/e6v2_*"))):
        tag = os.path.basename(d).replace("results/", "")
        models.append((tag, os.path.join(d, "final.pt")))
    rep = {}
    for tag, sd in models:
